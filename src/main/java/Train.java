...
public class Train {
	...
	public static void main(String[] args) throws Exception {
		...
		final byte[] graphDef = Files.readAllBytes(Paths.get(modelDir, "tensorflow_inception_graph.pb"));
		final String checkpointDir = "";
		final boolean checkpointExists = Files.exists(Paths.get(checkpointDir));

		try (Graph graph = new Graph();
				Session sess = new Session(graph);
				Tensor<String> checkpointPrefix = Tensors.create(Paths.get(checkpointDir, "ckpt").toString())) {
			...
			if (checkpointExists) {
				sess.runner().feed("input", checkpointPrefix).addTarget("input").run();
			} else {
			...
			// (Will be much more efficient if we sent batches instead of individual
			// values).
